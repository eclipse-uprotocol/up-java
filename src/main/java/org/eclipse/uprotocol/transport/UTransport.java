...
public interface UTransport {
    ...
    /**
     * Register {@code UListener} for {@code UUri} source filters to be called when a message is received.
     * 
     ...
     */
    default UStatus registerListener(UUri sourceFilter, UListener listener) {
        return registerListener(sourceFilter, null, listener);
    }


    /**
     * Register {@code UListener} for {@code UUri} source and sink filters to be called when a message is received.
     * 
     * @param sourceFilter The UAttributes::source address pattern that the message to receive needs to match.
     ...
     * correctly, otherwise it returns with the appropriate failure.
     */
    UStatus registerListener(UUri sourceFilter, UUri sinkFilter, UListener listener);


    /**
     * Unregister {@code UListener} for {@code UUri} source filters. Messages arriving on this topic will
     ...
     * correctly, otherwise it returns with the appropriate failure.
     */
    default UStatus unregisterListener(UUri sourceFilter, UListener listener) {
        return unregisterListener(sourceFilter, null, listener);
    }


    /**
     * Unregister {@code UListener} for {@code UUri} source and sink filters. Messages arriving on this topic will
     * no longer be processed by this listener.
     * 
     * @param sourceFilter The UAttributes::source address pattern that the message to receive needs to match.
     * @param sinkFilter The UAttributes::sink address pattern that the message to receive needs to match.
     ...
     * correctly, otherwise it returns with the appropriate failure.
     */
    UStatus unregisterListener(UUri sourceFilter, UUri sinkFilter, UListener listener);
